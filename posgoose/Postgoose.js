...
Postgoose.prototype.model = async function (name) {
  ...
  const rows = await this.isExist(name);
  if (!rows) {
    await pool.query(`create table ${name} (${this.schema})`);
...
Postgoose.prototype.create = async function (record) {
  ...
  if (this.virtual.length > 0 && created) {
    for (let i = 0; i < this.virtual.length; i++) {
      ...
      const rs = await CreateReusables(extract, v.table, pool);
    }
  }
...
Postgoose.prototype.isExist = async function (schema) {
